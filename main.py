...
# ==================== COMANDO LIMPIAR (NUEVO) ====================

@tree.command(name="multa_limpiar", description="Limpiar TODAS las multas de un usuario (Solo Staff)")
@app_commands.describe(miembro="Usuario al que se le van a limpiar las multas")
async def multa_limpiar(interaction: discord.Interaction, miembro: discord.Member):
    if not tiene_permiso(interaction, "staff"):
        return await interaction.response.send_message("❌ Solo el Staff puede usar este comando.", ephemeral=True)

    c.execute("DELETE FROM multas WHERE miembro_id=?", (miembro.id,))
    conn.commit()

    embed = discord.Embed(
        title="🧹 Multas Limpiadas",
        description=f"Se han eliminado **todas** las multas de {miembro.mention}.",
        color=0xFF69B4
    )
    await interaction.response.send_message(embed=embed)
